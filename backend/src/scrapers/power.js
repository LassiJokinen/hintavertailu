...
async function scrapePowerProduct(url) {
  const parsedUrl = new URL(url);
  const hostname = parsedUrl.hostname.toLowerCase().replace(/^www\./, "");

  if (!hostname.includes("power.fi")) {
    throw new Error("scrapePowerProduct only supports power.fi URLs");
  }
  ...
  if (!response.ok) {
    throw new Error(`Power request failed: ${response.status} ${response.statusText}`);
  }
