...
async function scrapeGiganttiProduct(url) {
  const parsedUrl = new URL(url);
  const hostname = parsedUrl.hostname.toLowerCase().replace(/^www\./, "");

  if (!hostname.includes("gigantti.fi")) {
    throw new Error("scrapeGiganttiProduct only supports gigantti.fi URLs");
  }
  ...
  if (!response.ok) {
    throw new Error(`Gigantti request failed: ${response.status} ${response.statusText}`);
  }
