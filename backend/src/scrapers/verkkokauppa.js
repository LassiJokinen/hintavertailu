...
async function scrapeVerkkokauppaProduct(url) {
  const parsedUrl = new URL(url);
  const hostname = parsedUrl.hostname.toLowerCase().replace(/^www\./, "");

  if (!hostname.includes("verkkokauppa.com")) {
    throw new Error("scrapeVerkkokauppaProduct only supports verkkokauppa.com URLs");
  }
  ...
  if (!response.ok) {
    throw new Error(`Verkkokauppa request failed: ${response.status} ${response.statusText}`);
  }
