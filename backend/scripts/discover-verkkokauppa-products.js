...
const STORE = "verkkokauppa.com";
const DEFAULT_SITEMAP_INDEX_URL = "https://www.verkkokauppa.com/gsitemaps1/sitemap.xml";
  ...
  try {
    ...
    for (const url of discovery.urls) {
      try {
        ...
        const product = await scrapeVerkkokauppaProduct(url);
    ...
    console.log("Verkkokauppa discovery complete.");
    console.log(JSON.stringify(summary, null, 2));
  } catch (error) {
    console.error("Verkkokauppa discovery failed:", error.message);
    process.exitCode = 1;
  } finally {
    await closeDb();
  }
...
async function discoverProductUrls(options, existingUrls) {
  ...
  const sitemapSources = await loadDiscoverySitemaps(options);
...
async function loadDiscoverySitemaps(options) {
  try {
    const xml = await fetchText(options.sitemapIndexUrl, options.delayMs);
    const sitemaps = parseLocUrls(xml)
      .map((url) => normalizeUrl(url))
      .filter(Boolean)
      .filter((url) => /\/products-\d+\.xml$/i.test(new URL(url).pathname) || /\/latest\.xml$/i.test(new URL(url).pathname))
      .slice(0, options.maxSitemapFiles);

    if (sitemaps.length) {
      return sitemaps;
    }
  } catch (error) {
    console.log(`Failed to load sitemap index, using fallback source: ${error.message}`);
  }

  return FALLBACK_STRUCTURED_SOURCES.slice(0, options.maxSitemapFiles);
}
...
function normalizeUrl(value) {
  ...
  try {
    ...
    if (parsed.hostname.toLowerCase().replace(/^www\./, "") !== STORE && !parsed.hostname.toLowerCase().includes("verkkokauppa.com")) {
      return null;
    }
...
function isRealProductUrl(url) {
  ...
  try {
    ...
    if (!parsed.pathname.includes("/fi/product/")) {
      return false;
    }

    return /^\/fi\/product\/[^/]+\/[^/]+/.test(parsed.pathname);
  } catch (error) {
    return false;
  }
}
