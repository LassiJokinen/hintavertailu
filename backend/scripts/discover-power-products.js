...
const STORE = "power.fi";
const DEFAULT_SITEMAP_INDEX_URL = "https://www.power.fi/services/sitemap.xml";
const FALLBACK_STRUCTURED_SOURCES = [
  "https://www.power.fi/api/new-spa-route/?path=/",
];
  ...
  try {
    ...
    for (const url of discovery.urls) {
      try {
        ...
        const product = await scrapePowerProduct(url);
    ...
    console.log("Power discovery complete.");
    console.log(JSON.stringify(summary, null, 2));
  } catch (error) {
    console.error("Power discovery failed:", error.message);
    process.exitCode = 1;
  } finally {
    await closeDb();
  }
...
async function discoverProductUrls(options, existingUrls) {
  ...
  const fallbackUrls = await discoverFromStructuredFallback(options, existingUrls);
...
async function loadSitemapSources(options) {
  const xml = await fetchText(options.sitemapIndexUrl, options.delayMs);
  ...
  return [options.sitemapIndexUrl].slice(0, options.maxSitemapFiles);
}
...
async function discoverFromStructuredFallback(options, existingUrls) {
  ...
  for (const sourceUrl of FALLBACK_STRUCTURED_SOURCES) {
    if (discovered.size >= options.maxProductUrls) {
      break;
    }

    console.log(`Scanning structured fallback source: ${sourceUrl}`);

    try {
      const raw = await fetchText(sourceUrl, options.delayMs);
      const candidates = extractFallbackProductUrls(raw);
      ...
      for (const url of candidates) {
        ...
        if (!isRealProductUrl(normalized)) {
          continue;
        }
      ...
      console.log(`  Skipped fallback source because it could not be read: ${error.message}`);
    }
  }
...
function extractFallbackProductUrls(raw) {
  const urls = [];
  const pattern = /"pid":"(\d+)","name":"((?:\\.|[^"\\])+)"/g;
  let match;

  while ((match = pattern.exec(raw)) !== null) {
    const pid = match[1];
    const decodedName = decodeJsonString(match[2]);
    const slug = slugifyForUrl(decodedName || "tuote");
    urls.push(`https://www.power.fi/${slug}/p-${pid}/`);
  }
